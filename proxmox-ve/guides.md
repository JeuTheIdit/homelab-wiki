...
The easiest way to update the firmware is by installing the latest drivers on Windows, which the firmware updates are packaged with. Either install it in a standalone Windows 11 machine, or [create a Windows 11 VM](https://github.com/JeuTheIdit/homelab-wiki/blob/main/proxmox-ve/tips.md#windows-guest-best-practices) and [passthrough the GPU](#gpu-passthrough).
